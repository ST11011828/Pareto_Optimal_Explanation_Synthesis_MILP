...
def sample_constraints(model,m , S, lam , tau , I ,  P , C , L , b , d):
    for s in S:
        ...
        for i in I:
            ...
            for c in C:
                model.addConstr( b[(i,c,s)] == gp.quicksum(func(s,p,c)*lam[i,p] for p in P) , name="forming b")
                for j in list(I) + list(L.keys()):
                    if node_order(j)>i:
                        model.addConstr(b[(i,c,s)] + m[(j,s)] + tau[(i,c,j)] - 2 <= m[(i,s)], name ="m lower bound")
                        model.addConstr(d[(i,c,j,s)] >= b[(i,c,s)] + m[(j,s)] + tau[(i,c,j)]-2 , name = " d upper bound")
...
def reachability_constraints(model, lam, tau, u , z_u , I , C , P , o_u , L):
    ...
    # #recursively balelling the tree top down
    for j in list(I)+list(L.keys()):
        for i in I:
            if i<node_order(j):
                for c in C:
    ...
    model.addConstr(gp.quicksum( 1- u[i] for i in I) + gp.quicksum(o_u[i,p] for i in I for p in P)<=MAX_EXPLANATION)

def objective(model, u , o_u , m , I , P , S):
    model.setObjective((gp.quicksum( 1- u[i] for i in I) + gp.quicksum(o_u[i,p] for i in I for p in P) + gp.quicksum(m[0,s] for s in S)), GRB.MAXIMIZE)
