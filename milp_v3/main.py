#TODO: wRITE A MAIN FUNCTION that call all the encodings and optimizes the model accordingly and prints the optimal values
import gurobipy as gp
from gurobipy import GRB
from encoding import node_order,tree_constraints, sample_constraints, reachability_constraints, objective
from helper_functions import read_features,num_buckets, func , df
...
def main():
    ...
    # lam = model.addVars(I,P, vtype=GRB.CONTINUOUS ,lb = 0.0, ub = 1.0, name = "lam")
    # Continuous for node 0
    ...
    # INTEGER tau for i = 0
    tau0 = model.addVars(((0,c,j) for c in C for j in list(I)+list(L.keys()) if node_order(j) > 0),
                        vtype=GRB.INTEGER, lb=0.0, ub=1.0, name="tau")

    # INTEGER tau for i > 0
    tau_rest = model.addVars(((i,c,j) for i in I if i > 0 for c in C for j in list(I)+list(L.keys()) if node_order(j) > i),
    ...
    sample_constraints(model,m,S,lam,tau,I,P,C,L,b,d)
    ...
    if model.status == GRB.OPTIMAL or model.status == GRB.SUBOPTIMAL:
        ...
        for v in model.getVars():
            x = v.X
            if x is None:  # model not solved yet
                continue
    ...
    print( "________________________________________________________________________", elapsed_time)
