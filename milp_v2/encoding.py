...
from helper_functions import func, return_weight
...
def objective(model, u , o_u , m , I , P , S):
    model.setObjective((gp.quicksum( 1- u[i] for i in I) + gp.quicksum(return_weight(p)*o_u[i,p] for i in I for p in P) + gp.quicksum(m[0,s] for s in S)/len(S)), GRB.MAXIMIZE)
